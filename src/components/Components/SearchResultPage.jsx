...
const SearchResultPage = () => {
  ...
  const [filter, setFilter] = useState(searchTerm); // Set initial filter value to searchTerm
  ...
  useEffect(() => {
    // Trigger the search function whenever the filter value changes
    handleSearch();
  }, [filter]); // Update the dependency to filter

  const handleInputChange = (e) => {
    const value = e.target.value;
    setSearchInputValue(value);
    setFilter(value); // Set the filter value directly based on the input value
  };

  const handleSearch = () => {
    if (filter.trim() !== '') { // Use filter instead of searchInputValue
